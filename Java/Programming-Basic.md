...
- *** `char c2 = c1 + 1;` > 연산 결과값이 int로 컴파일 오류 발생 
`char c2 = 'a' + 1;` > 상수 또는 리터럴간의 연산은 실행과정동안 변하는 값이 아님
- *** 나머지 연산자(%)의 나누는 수로 음수도 허용하나, 부호 무시! `10 % -8 = 2`
...
- `0.1 == 0.1f` > **false**  > 부동 소수점 방식 > 0.1의 근사값으로 저장됨
